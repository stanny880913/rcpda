...
from RadarPointCloud import RadarPointCloud
...
def merge_selected_radar(sample_idx, frame_range=[0, 12]):
    '''
    ...
    def cal_matrix_refCam_from_global(cam_data):
        ...
        ref_pose_rec_tran = [1009.9756887158287, 615.0979208957995, 0.0]
        ref_pose_rec_ro = [-0.6158940620859793, -0.007091006156514636,
                           0.007416135903410572, -0.7877621613451498]
        ref_cs_rec_tran = [1.70079118954, 0.0159456324149, 1.51095763913]
        ref_cs_rec_ro = [0.4998015430569128, -0.5030316162024876,
                         0.4997798114386805, -0.49737083824542755]
        # ref_from_car = transform_matrix(ref_cs_rec['translation'], Quaternion(ref_cs_rec['rotation']), inverse=True)
        # car_from_global = transform_matrix(ref_pose_rec['translation'], Quaternion(ref_pose_rec['rotation']), inverse=True)
        ref_from_car = transform_matrix(
            ref_cs_rec_tran, Quaternion(ref_cs_rec_ro), inverse=True)
        car_from_global = transform_matrix(
            ref_pose_rec_tran, Quaternion(ref_pose_rec_ro), inverse=True)
    ...
    def current_2_ref(current_sd_rec, M_refCam_from_global, ref_time, min_distance=1):
        ...
        ref_cs_rec_ro = [0.4998015430569128, -0.5030316162024876,
                         0.4997798114386805, -0.49737083824542755]
        # current_pc = pc.from_file(
        #     osp.join(nusc.dataroot, current_sd_rec['filename']))
        current_pc = pc.from_file(osp.join(pcd_dir, current_sd_rec))
        ...
        car_from_current = transform_matrix(
            ref_cs_rec_tran, Quaternion(ref_cs_rec_ro), inverse=False)
        global_from_car = transform_matrix(
            ref_pose_rec_tran, Quaternion(ref_pose_rec_ro), inverse=True)

        trans_matrix = reduce(
            np.dot, [M_refCam_from_global, global_from_car, car_from_current])

        # time_lag = ref_time - 1e-6 * current_pc['timestamp']
        time_lag = 0
    ...
    def proj2im(pc_cam, cam_data, min_z=2):
        ...
        # points = view_points(pc_cam.points[:3, :], np.array(
        # cs_rec['camera_intrinsic']), normalize=True)

        points = view_points(pc_cam.points[:3, :], np.array(K), normalize=True)
    ...
    pcd_folder_path = '/media/stannyho/ssd/rc-pda/data_own/pcd'  # 替换为您的文件夹路径
    img_folder_path = '/media/stannyho/ssd/rc-pda/data_own/img_jpg'
    pcd_files = []  # 存储文件名的列表
    ...
    pcl_path = os.path.join(pcd_folder_path, pcd_files[1])
    ...
    idx = 0
    # for idx,img_file_name in enumerate(img_files):
    cam_data = img_files[idx]
    if idx < len(img_files) - 1:
        cam_data1 = img_files[idx+1]
        cam_data2 = img_files[idx+2]
    ...
    while k < frame_range[0]:
        ...
        if idx == 0:
            ...
            current_sd_rec = pcd_files[idx - 1]
        k += 1
    ...
    for _ in range(nsweeps):
        ...
        current_pc1, time_lag1 = current_2_ref(
            current_sd_rec, M_refCam1_from_global, ref_time1)
        current_pc2, time_lag2 = current_2_ref(
            current_sd_rec, M_refCam2_from_global, ref_time2)
    ...
    x1, y1, x2, y2, depth1, depth2, all_times1, all_times2, rcs, v_comp = x1[msk], y1[msk], x2[
        msk], y2[msk], depth1[msk], depth2[msk], all_times1[msk], all_times2[msk], rcs[msk], v_comp[msk]
...
def cal_depthMap_flow(x1, y1, depth1, all_times1, x2, y2, depth2, all_times2, rcs, v_comp, downsample_scale, y_cutoff):
    ...
    time_map1, rcs_map1, v_comp_map1 = time_map1[y_cutoff:,
                                                 ...], rcs_map1[y_cutoff:, ...], v_comp_map1[y_cutoff:, ...]
...
def downsample_im(im, downsample_scale, y_cutoff):
    ...
    im = resize(im, (h_im, w_im, 3), order=1,
...
def plt_flow_on_im(flow, im, skip=3):
    ...
    for i in range(0, h, skip+1):
        for j in range(0, w, skip+1):
            if msk[i, j]:
                plt.arrow(j, i, flow[i, j, 0], flow[i, j, 1],
                          length_includes_head=True, width=0.05, head_width=0.5, color='cyan')

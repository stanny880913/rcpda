...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        d_radar = self.radar_list[idx][..., [0]].astype(
            'float32').transpose((2, 0, 1))      # (1,h,w)
        ...
        duv_im = (uvt2_im - uv1_map) * \
            scale_factor                    # optical flow
        duv_radar = (uvt2_radar - uv1_map) * msk_radar * \
            scale_factor  # radar flow
...
if __name__ == '__main__':
    ...
    print('data_in', sample['data_in'].shape, type(
        sample['data_in']), sample['data_in'].dtype)
    print('d_lidar', sample['d_lidar'].shape, type(
        sample['d_lidar']), sample['d_lidar'].dtype)
    print('d_lidar', sample['d_radar'].shape, type(
        sample['d_radar']), sample['d_radar'].dtype)

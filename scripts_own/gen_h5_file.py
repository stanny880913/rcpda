...
def create_data_group(hf, mode, sample_indices):
    ...
    uv2_im_list = []
    radar_list = []
    sample_indices_len = len(sample_indices)
    # for idx in tqdm(sample_indices, '%s:im' % mode):
    for idx,sample_indices in enumerate(tqdm(sample_indices, '%s:total' % mode)):
        if(idx <= sample_indices_len - 3):
    ...
    # uv2_im_list = []
    # for idx,sample_indices in enumerate(tqdm(sample_indices, '%s:im' % mode)):
    #     if(idx <= sample_indices_len - 3):
    #         test2 = join(dir_label, '%05d_im_uv.npy' % idx) 
    #         uv2_im = np.load(join(dir_label, '%05d_im_uv.npy' % idx))
    #         uv2_im_list.append(uv2_im) 
    # group.create_dataset('im_uv',data=np.array(uv2_im_list))
    # del uv2_im_list
    ...
    # for idx,sample_indices in enumerate(tqdm(sample_indices, '%s:radar' % mode)):
    #     if(idx <= sample_indices_len - 3):
    #         radar = np.load(join(dir_label, '%05d_radar.npy' % idx) ).astype('float32')  # d_radar, u2, v2       
    #         radar_list.append(radar)
    # group.create_dataset('radar',data=np.array(radar_list))
...
if __name__ == '__main__':
    ...
    path_h5_file = join(args.dir_data, 'own_prepared_data.h5')
    ...
    own_sample_indices = torch.load(join(args.dir_data,'own_data_split.tar'))['own_sample_files']
    ...
    create_data_group(hf,'own',own_sample_indices) 
    hf.close()

...
from data_loader_depth_own import init_data_loader
    ...
    with torch.no_grad():
        for ct, sample in enumerate(test_loader):
            if ct == idx:
                ...
                im = data_in[0][0:3, ...].permute(
                    1, 2, 0).to('cpu').numpy().astype('uint8')
...
def main(args):
    ...
    if args.dir_data == None:
        ...
        args.dir_data = join(this_dir, '..', 'data_own')

    if not args.dir_result:
        args.dir_result = join(
            args.dir_data, 'train_result', 'depth_completion')

    args.path_data_file = join(args.dir_data, 'own_prepared_data.h5')

    args.path_radar_file = join(args.dir_data, 'own_mer_2_30_5_0.5.h5')

    args.batch_size = 1

    device = init_env()

    own_loader = init_data_loader(args, 'own')
    ...
    test_len = len(own_loader)

    test_save_path = "/media/stannyho/ssd/rc-pda/inference/own"
    ...
    for idx in tqdm(range(test_len)):
        prd_one_sample(model, own_loader, device, idx, args, test_save_path)

    # evaluation
    # evaluate(model, own_loader, device, d_min=1e-3, d_max=args.d_max, eval_low_height = False)

    print('\n Low height')
    # evaluate(model, own_loader, device, d_min=1e-3, d_max=args.d_max, eval_low_height = True)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--dir_data', type=str)
    parser.add_argument('--dir_result', default='/media/stannyho/ssd/rc-pda/models/test_depth', type=str)
    ...
    parser.add_argument('--outChannels', type=int, default=1,
                        help='number of output channel of network; automatically set to 1 if pred_task is foreground_seg')

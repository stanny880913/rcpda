...
from data_loader_pda_own import init_data_loader
...
def create_data_group(hf, mode, loader, device, nb, model, thres_affs):
    ...
    with torch.no_grad():
        for ct, sample in enumerate(tqdm(loader, '%s' % mode)):
            data_in, d_radar = sample['data_in'].to(
                device), sample['d_radar'].to(device)
            prd = torch.sigmoid(model(data_in)[0])

            d_est, aff = cal_enhanced_depth_with_max_aff(
                prd, d_radar, nb, device, thres_affs[0])
...
def main(args):
    ...
    if not args.dir_result:
        args.dir_result = join(args.dir_data, 'train_result', 'pda_%d_%d_%d' % (
            args.left_right, args.top, args.bottom))
    ...
    nb = neighbor_connection(
        *(args.left_right, args.left_right, args.top, args.bottom))

    path_h5_file = join(args.dir_data, 'own_mer_%d_%d_%d_%.1f.h5' % (
        args.left_right, args.top, args.bottom, thres_affs[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_data_shuffle', type=bool, default=True,
                        help='for generating training data in order')

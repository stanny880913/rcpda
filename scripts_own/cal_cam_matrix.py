...
def current_2_ref_matrix(nusc, current_sd_token, ref_sd_token):
    ...
    car_from_global = transform_matrix(ref_pose_rec['translation'], Quaternion(
        ref_pose_rec['rotation']), inverse=True)
    ...
    M_ref_from_current = reduce(
        np.dot, [M_ref_from_global, M_global_from_current])
...
if __name__ == '__main__':
    ...
    own_sample_indices = torch.load(
        join(args.dir_data, 'own_data_split.tar'))['all_indices']
    ...
    for idx, sample_idx in enumerate(own_sample_indices):
        ...
        cam_token1 = own_sample_indices[idx+1]

        # if own_sample_indices[idx+2]:
        cam_token2 = own_sample_indices[idx+2]
        ...
        print('Save matrix %d/%d' % (ct, len(own_sample_indices)))
